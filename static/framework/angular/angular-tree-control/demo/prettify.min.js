...
!function() {
        ...
        function d(a) {
            ...
            for (var h = f ? 1 : 0, i = d.length; h < i; ++h) {
                ...
                if (/\\[bdsw]/i.test(j)) g.push(j); else {
                    var k, l = b(j);
                    h + 2 < i && "-" === d[h + 1] ? (k = b(d[h + 2]), h += 2) : k = l, e.push([ l, k ]), 
                    k < 65 || l > 122 || (k < 65 || l > 90 || e.push([ 32 | Math.max(65, l), 32 | Math.min(k, 90) ]), 
                    k < 97 || l > 122 || e.push([ Math.max(97, l) & -33, Math.min(k, 122) & -33 ]));
                }
            }
            e.sort(function(a, b) {
        ...
        function c(a) {
            switch (a.nodeType) {
              ...
              case 4:
                ...
                e[h] = k, g[h << 1] = f, f += k.length, g[h++ << 1 | 1] = a);
            }
        }
    ...
    function e(b, d) {
        ...
        var g = d.length, h = function(a) {
            for (var b = a.sourceCode, i = a.basePos, k = [ i, I ], l = 0, m = b.match(e) || [], n = {}, o = 0, p = m.length; o < p; ++o) {
    ...
    function g(a, b, c) {
        ...
        for (var m = Math.max(0, b - 1 | 0) || 0, k = 0, n = j.length; k < n; ++k) i = j[k], 
        ...
        try {
            for (;i < h; ) {
                ...
                if (1 !== w.nodeType && (t = d.substring(f, q))) {
                    ...
                    y.className = j[l + 1];
                    var z = w.parentNode;
                    z.replaceChild(y, w), y.appendChild(w), f < u && (g[i + 1] = w = x.createTextNode(d.substring(q, u)), 
    ...
    function m(a) {
            ...
            for (var b = n.PR_SHOULD_USE_CONTINUATION ? l.now() + 250 : 1 / 0; o < f.length && l.now() < b; o++) {
                ...
                if (q.test(h) && !r.test(h)) {
                    ...
                    if (!i) {
                        ...
                        B = !!B && (!B[1] || !B[1].length || +B[1]), B && g(e, B, y), m = {
                            langExtension: x,
                            sourceNode: e,
                            numberLines: B,
                            pre: y
        ...
        for (var e = [ b("pre"), b("code"), b("xmp") ], f = [], h = 0; h < e.length; ++h) for (var i = 0, j = e[h].length; i < j; ++i) f.push(e[h][i]);
        e = null;
        var l = Date;
        l.now || (l = {
            now: function() {
