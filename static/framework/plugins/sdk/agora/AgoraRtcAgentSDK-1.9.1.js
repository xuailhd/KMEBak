...
function AgoraRender() {
    ...
    that.renderImage = function (image) {
        ...
        if (image.width != that.initWidth || image.height != that.initHeight || image.rotation != that.initRotation || image.mirror != that.mirrorView) {
            ...
            initCanvas(view, image.mirror, image.width, image.height, image.rotation,
            function (e) {
...
AgoraRTC.secureAgentUrl = "wss://localhost.agora.io:8921/";
AgoraRTC.agentUrl = "ws://127.0.0.1:8922/";
AgoraRTC.getAgentUrl = function () {
    if (location.protocol === "https:") {
        return AgoraRTC.secureAgentUrl
    }
    return AgoraRTC.agentUrl
};
AgoraRTC.macAgentInstallUrl = "http://download.agora.io/sdk/release/AgoraWebAgent-1.9.0.pkg";
AgoraRTC.winAgentInstallUrl = "http://download.agora.io/sdk/release/AgoraWebAgentSetup-1.9.0.exe";
...
AgoraRTC.Signal = function (spec) {
    ...
    that.connection.onopen = function (e) {
        console.log(e);
        that.dispatchEvent(AgoraRTC.MediaEvent({
            type: "onopen",
            event: e
        }))
    };
    ...
    that.connection.onmessage = function (e) {
        ...
        } else if (m.hasOwnProperty("event")) {
            that.onEvent(m)
        }
    };
    function onEvent(e) {
...
AgoraRTC.Stream = function (spec) {
    ...
    that.init = function (onSuccess, onFailure) {
        ...
        var videoStreamFileBlob = new Blob([AgoraRTC.videoStreamSource], {
    ...
    that.play = function (div, onFailure) {
        ...
        that.render.start(view, onFailure);
        if (spec.local) {
            that.stream.postMessage({
                type: "send",
                message: {
                    command: "preview",
                    uid: spec.streamID,
                    audio: spec.audio === false ? false : true,
                    video: spec.video === false ? false : true,
                    screen: spec.screen === true ? true : false,
    ...
    that.setVideoProfile = function (profile) {
        that.profile = {};
        if (typeof profile !== "string" && typeof profile !== "object") {
            return false
        }
        if (profile.hasOwnProperty("profile")) {
            that.profile.profile = profile.profile
        }
        if (profile.hasOwnProperty("swapWidthAndHeight")) {
            that.profile.swapWidthAndHeight = profile.swapWidthAndHeight !== false
        } else {
            that.profile.swapWidthAndHeight = false
        }
        ...
        if (that.stream) {
            that.stream.postMessage({
                ...
                message: {
                    ...
                    swapWidthAndHeight: that.profile.swapWidthAndHeight
                }
            })
        }
        return true
...
AgoraRTC.Client = function (spec) {
    ...
    that.agentUrl = AgoraRTC.getAgentUrl();
    function onSignalError(e) {
        if (that.signalOpen) {
            ...
            AgoraCall(that.onFailure, message);
            that.dispatchEvent(message);
            return
        }
        if (e.type === "error") {
            return
    ...
    that.init = function (appId, onSuccess, onFailure) {
        ...
        if (!that.signal) {
            ...
            function (event) {
                ...
                that.signal.onEvent = function (e) {
                    if (e.event === "error") {
                        var name = getErrorCodeName(e.code);
                        if (name) {
                            e.reason = name
                        }
                    } else if (e.event === "onUserMuteAudio") {
                        e.event = "peer-mute-audio"
                    } else if (e.event === "onUserMuteVideo") {
                        e.event = "peer-mute-video"
                    }
                    that.dispatchEvent(AgoraRTC.MediaEvent({
                        type: e.event,
                        msg: e
                ...
                that.getVersion(function (ev) {
                    if (ev.version == "1.6.0" || ev.version == "1.6.1" || ev.version == "1.7.0" || ev.version == "1.7.1" || ev.version == "1.7.2" || ev.version == "1.7.3" || ev.version == "1.7.4" || ev.version == "1.8.0" || ev.version == "1.8.1" || ev.version == "1.9.0") {
        ...
        function initialize(onSuccess, onFailure) {
            sendSignalCommand({
                command: "initialize",
                appId: spec.appId
            },
        ...
        function onError(code, type) {
            var name = getErrorCodeName(code);
            if (!name) {
                name = "UNKNOWN_ERROR"
            }
            notifyError({
    ...
    that.join = function (channelKey, channelName, uid, onSuccess, onFailure) {
    ...
    that.startRecording = function (recordingKey, onSuccess, onFailure) {
        sendSignalCommand({
    ...
    that.stopRecording = function (recordingKey, onSuccess, onFailure) {
        sendSignalCommand({
    ...
    that.setEncryptionMode = function (encryptionMode) {
        sendSignalCommand({
    ...
    that.setChannelProfile = function (profile, onSuccess, onFailure) {
        sendSignalCommand({
    ...
    that.setClientRole = function (role, permissionKey, onSuccess, onFailure) {
        if (!permissionKey) {
            permissionKey = ""
        }
        sendSignalCommand({
    ...
    that.enableDualStreamMode = function (enabled, onSuccess, onFailure) {
        sendSignalCommand({
    ...
    function sendSignalCommand(message, onSuccess, onFailure) {
        that.signal.on(message.command,
        function (e) {
            AgoraCall(onSuccess, e);
            console.log(e)
        });
        that.signal.sendMessage(message,
        function (e) {
            console.log(e);
