!function() {
    function a() {
        for (var a = $G("tabHeads").children, b = 0; b < a.length; b++) domUtils.on(a[b], "click", function(b) {
            ...
            for (c = 0; c < a.length; c++) d = a[c].getAttribute("data-content-id"), a[c] == e ? (domUtils.addClass(a[c], "focus"), 
            domUtils.addClass($G(d), "focus")) : (domUtils.removeClasses(a[c], "focus"), domUtils.removeClasses($G(d), "focus"));
        });
    }
    function b() {
        k([ "videoFloat", "upload_alignment" ]), m($G("videoUrl")), c(), function() {
            var a, b = editor.selection.getRange().getClosedNode();
            if (b && b.className) {
                var c = "edui-faked-video" == b.className, e = b.className.indexOf("edui-upload-video") != -1;
                if (c || e) {
                    $G("videoUrl").value = a = b.getAttribute("_url"), $G("videoWidth").value = b.width, 
    ...
    function e() {
        var a = $G("videoWidth"), b = $G("videoHeight"), c = $G("videoUrl").value, d = g("videoFloat", "name");
        return !!c && (!!i([ a, b ]) && void editor.execCommand("insertvideo", {
            url: h(c),
            width: a.value,
            height: b.value,
            align: d
    ...
    function k(a) {
        for (var b, c = 0; b = a[c++]; ) {
            ...
            for (var f in e) {
                ...
                g.setAttribute("name", f), "none" == f && (g.className = "focus"), g.style.cssText = "background:url(images/" + f + "_focus.jpg);", 
                g.setAttribute("title", e[f]), d.appendChild(g);
            }
            l(b);
        }
    }
    ...
    function o() {
        ...
        var i = r.getQueueCount();
        return i ? ($(".info", "#queueList").html('<span style="color:red;">' + "还有2个未上传文件".replace(/[\d]/, i) + "</span>"), 
        !1) : void editor.execCommand("insertvideo", a, "upload");
    }
    function p() {
        r = new q("queueList");
    }
    ...
    window.onload = function() {
        ...
        initUploader: function() {
            function a(a) {
                ...
                "invalid" === a.getStatus() ? i(a.statusText) : (e.text(lang.uploadPreview), "|png|jpg|jpeg|bmp|gif|".indexOf("|" + a.ext.toLowerCase() + "|") == -1 ? e.empty().addClass("notimage").append('<i class="file-preview file-type-' + a.ext.toLowerCase() + '"></i><span class="file-title">' + a.name + "</span>") : browser.ie && browser.version <= 7 ? e.text(lang.uploadNoPreview) : f.makeThumb(a, function(a, b) {
                ...
                }, u, v), x[a.id] = [ a.size, 0 ], a.rotation = 0, a.ext && B.indexOf(a.ext.toLowerCase()) != -1 || (i("not_allow_type"), 
                f.removeFile(a))), a.on("statuschange", function(e, f) {
                    "progress" === f ? d.hide().width(0) : "queued" === f && (b.off("mouseenter mouseleave"), 
                    c.remove()), "error" === e || "invalid" === e ? (i(a.statusText), x[a.id][1] = 1) : "interrupt" === e ? i("interrupt") : "queued" === e ? x[a.id][1] = 0 : "progress" === e && (g.hide(), 
            ...
            function b(a) {
                var b = h("#" + a.id);
                delete x[a.id], c(), b.off().find(".file-panel").off().end().remove();
            }
            function c() {
                var a, b = 0, c = 0, d = p.children();
                h.each(x, function(a, d) {
                    c += d[0], b += d[0] * d[1];
            ...
            function d(a, b) {
                if (a != w) {
                    var c = f.getStats();
                    switch (m.removeClass("state-" + w), m.addClass("state-" + a), a) {
                      case "pedding":
            ...
            function e() {
                var a, b = "";
                "ready" === w ? b = lang.updateStatusReady.replace("_", q).replace("_KB", WebUploader.formatSize(r)) : "confirm" === w ? (a = f.getStats(), 
            ...
            i.find(".filePickerBlock")), o = i.find(".placeholder"), p = k.find(".progress").hide(), q = 0, r = 0, t = window.devicePixelRatio || 1, u = 113 * t, v = 113 * t, w = "", x = {}, y = function() {
            ...
            return WebUploader.Uploader.support() ? editor.getOpt("videoActionName") ? (f = g.uploader = WebUploader.create({
                ...
                switch (a) {
                  ...
                  case "startUpload":
                    var c = utils.serializeParam(editor.queryCommandValue("serverparam")) || "", e = utils.formatUrl(z + (z.indexOf("?") == -1 ? "?" : "&") + "encode=utf-8&" + c);
            ...
            }), m.addClass("state-" + w), void c()) : void h("#filePickerReady").after(h("<div>").html(lang.errorLoadConfig)).hide() : void h("#filePickerReady").after(h("<div>").html(lang.errorNotSupport)).hide();
        },
