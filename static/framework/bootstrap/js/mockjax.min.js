!function(a) {
    ...
    function d(b, c) {
        var e = !1;
        return "string" == typeof c ? a.isFunction(b.test) ? b.test(c) : b == c : (a.each(b, function(f, g) {
    ...
    function e(b, c) {
        ...
        if (a.isFunction(b.url.test)) {
            ...
            var e = b.url.indexOf("*");
            if (b.url !== c.url && e === -1 || !new RegExp(b.url.replace(/[-[\]{}()+?.,\\^$|#\s]/g, "\\$&").replace("*", ".+")).test(c.url)) return null;
        }
        ...
        var f = function(f) {
            return function() {
                return function() {
                    ...
                    "json" == d.dataType && "object" == typeof c.responseText ? this.responseText = JSON.stringify(c.responseText) : "xml" == d.dataType ? "string" == typeof c.responseXML ? this.responseXML = b(c.responseXML) : this.responseXML = c.responseXML : this.responseText = c.responseText, 
                    "number" != typeof c.status && "string" != typeof c.status || (this.status = c.status), 
                    "string" == typeof c.statusText && (this.statusText = c.statusText), a.isFunction(this.onreadystatechange) ? (c.isTimeout && (this.status = -1), 
                    this.onreadystatechange(c.isTimeout ? "timeout" : void 0)) : c.isTimeout && (this.status = -1);
                }.apply(f);
            };
        }(this);
        c.proxy ? p({
            ...
            complete: function(a, b) {
                c.responseXML = a.responseXML, c.responseText = a.responseText, c.status = a.status, 
                c.statusText = a.statusText, this.responseTimer = setTimeout(f, c.responseTime || 0);
            }
        }) : d.async === !1 ? f() : this.responseTimer = setTimeout(f, c.responseTime || 50);
    }
    function h(b, c, d, e) {
        return b = a.extend({}, a.mockjaxSettings, b), "undefined" == typeof b.headers && (b.headers = {}), 
    ...
    function k(b, c, d) {
        ...
        return c.response && a.isFunction(c.response) ? c.response(d) : "object" == typeof c.responseText ? a.globalEval("(" + JSON.stringify(c.responseText) + ")") : a.globalEval("(" + c.responseText + ")"), 
        m(b, c), n(b, c), jQuery.Deferred && (e = new jQuery.Deferred(), "object" == typeof c.responseText ? e.resolve(c.responseText) : e.resolve(jQuery.parseJSON(c.responseText))), 
        e;
    }
    function l(a, b) {
        jsonp = a.jsonpCallback || "jsonp" + s++, a.data && (a.data = (a.data + "").replace(r, "=" + jsonp + "$1")), 
        a.url = a.url.replace(r, "=" + jsonp + "$1"), window[jsonp] = window[jsonp] || function(c) {
    ...
    function m(a, b) {
        a.success && a.success.call(callbackContext, b.response ? b.response.toString() : b.responseText || "", status, {}), 
        a.global && c(a, "ajaxSuccess", [ {}, a ]);
    }
    function n(a, b) {
        a.complete && a.complete.call(callbackContext, {}, status), a.global && c("ajaxComplete", [ {}, a ]), 
    ...
    function o(b, c) {
        ...
        for (var k = 0; k < q.length; k++) if (q[k] && (j = e(q[k], g))) return f(j, g), 
        "jsonp" === g.dataType && (d = i(g, j, c)) ? d : (j.cache = g.cache, j.timeout = g.timeout, 
        j.global = g.global, function(b, c, e, f) {
            d = p.call(a, a.extend(!0, {}, e, {
                xhr: function() {
                    return h(b, c, e, f);
                }
            }));
    ...
    a.extend({
        ...
        if (1 == arguments.length) return q[a];
    };
}(jQuery);
