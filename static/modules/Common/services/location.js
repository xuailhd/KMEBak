define([], function () {
    ...
    function find(filter) {
        ...
        for (var i = 0; i < Location.length; i++) {
            if (filter.province)
            {
                if (filter.city)
                {
                    if (Location[i].name == filter.province)
                    {
                        for (var j = 0; j < Location[i].cityList.length; j++)
                        {
                            
                            if (Location[i].cityList[j].name == filter.city)
                            {
                             
                                for (var k = 0; k < Location[i].cityList[j].areaList.length; k++) {

                         
                                    //返回地区
                                    result.push(Location[i].cityList[j].areaList[k]);
                                }
                            }
                        }
                    }
                    ...
                    if (Location[i].name == filter.province)
                    {
                        for (var j = 0; j < Location[i].cityList.length; j++) {
                            //返回城市列表
                            result.push(Location[i].cityList[j].name);
                        }
                    }
    ...
    function fillOption(el, filter) {

        var selected = el.selected;
 
        filter = filter || {};

        var json = find(filter);
    ...
    function render(loc_province, loc_city, loc_town) {

   

        var title = [loc_province.attr("placeholder") || "省份", loc_province.attr("loc_city") ||"市", loc_province.attr("loc_town") || "区、县"];
        ...
        loc_province.change(function () {
            loc_city.empty();
            loc_city.append(title[1]);
            fillOption(loc_city, { province: loc_province.val() });
            loc_city.change()
        })
        loc_city.change(function () {
            loc_town.empty();
            loc_town.append(title[2]);
            fillOption(loc_town, { province: loc_province.val(), city: loc_city.val() });
        })

        if (loc_province.selected != "") {
            fillOption(loc_province, {});

            if (loc_city.selected != "") {
                fillOption(loc_city, { province: loc_province.selected });

                if (loc_town.selected != "") {
                    fillOption(loc_town, { province: loc_province.selected, city: loc_city.selected });
                }
            }
        }
        else
